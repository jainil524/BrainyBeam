...
const Dropdown = ({ id, label, options, value, onChange, error, classes = [], validate, ...additionalProps }) => {
    ...
    const handleChange = (e) => {
        ...
        if (validate) {
            const validationError = validate(selectedValue);
            setValidationError(validationError);
        } else {
            setValidationError(''); // Clear error if no validation
        }
    };
    ...
    return (
        <div className="input-wrapper">
            ...
            <select
                ...
                className={`border rounded p-2 ${validationError ? 'border-red-500' : 'border-gray-300'} ${classes.join(' ')}`}
                {...additionalProps}
            >
            ...
            {(error || validationError) && <span className="error">{error || validationError}</span>}
        </div>
    );
};
